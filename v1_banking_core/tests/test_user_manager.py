...
import pytest
from banking_core.utils import validate_name, validate_phone, validate_email, validate_age, get_age_group
...
class TestBankAliases:
    def test_sbi_alias(self):
        from banking_core.utils import resolve_bank_alias
...
class TestAccountGeneration:
    def test_account_no_format(self):
        from banking_core.utils import generate_account_no
        acc = generate_account_no("SBI")
        assert acc.startswith("SBI")
        assert len(acc) > 3

    def test_card_no_format(self):
        from banking_core.utils import generate_card_no
