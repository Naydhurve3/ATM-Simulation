...
from banking_core.utils import ECOSYSTEM_DB, DATA_TRAINING, get_scenario_timestamp, ensure_dirs
from banking_core.data.db_manager import db
from banking_core.data.data_catalog import DataCatalog
        ...
        try:
            df = pd.read_sql("""
                SELECT u.user_id, u.age, u.age_group, u.is_minor,
                       ...
                       CAST(COALESCE(SUM(CASE WHEN t.is_fraud=TRUE THEN 1 ELSE 0 END),0) AS INTEGER) as fraud_count
                FROM users u
        ...
        try:
            df = pd.read_sql("""
                SELECT bank, account_type,
                       ...
                       SUM(CASE WHEN is_minor=TRUE THEN 1 ELSE 0 END) as minor_count
                FROM users
