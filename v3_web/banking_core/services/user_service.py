...
from banking_core.utils import (
    hash_pin, verify_pin, get_age_group, generate_account_no, generate_card_no, ensure_dirs,
)
...
class UserService:
    ...
    def _init_db(self):
        ...
        self.conn.commit()
        try:
            self.conn.cursor().execute("ALTER TABLE user_sessions ADD COLUMN token TEXT")
            self.conn.commit()
    ...
    def authenticate(self, identifier: str, pin: str, token: str = None):
        ...
        if not user["pin_hash"].startswith("pbkdf2$"):
            c = self.conn.cursor()
            c.execute("UPDATE users SET pin_hash = ? WHERE user_id = ?", (hash_pin(pin), user["user_id"]))
            self.conn.commit()
        user["is_minor"] = bool(user["is_minor"])
        self.log_session(user["user_id"], token)
        return user
    ...
    def register(self, data: dict):
        ...
        try:
            ...
            row = c.fetchone() if c.description else None
            self.conn.commit()
            user_data["user_id"] = c.lastrowid or (row[0] if row else None)
    ...
    def log_session(self, user_id: int, token: str = None):
        c = self.conn.cursor()
        c.execute("INSERT INTO user_sessions (user_id, token) VALUES (?, ?)", (user_id, token))
        self.conn.commit()

    def end_session(self, user_id: int, token: str = None):
        c = self.conn.cursor()
        if token:
            c.execute("""UPDATE user_sessions SET logout_at = CURRENT_TIMESTAMP
                WHERE user_id = ? AND token = ? AND logout_at IS NULL""", (user_id, token))
        else:
            c.execute("""UPDATE user_sessions SET logout_at = CURRENT_TIMESTAMP
    ...
    def is_session_valid(self, user_id: int, token: str) -> bool:
        if not token:
            return False
        c = self.conn.cursor()
        c.execute("""SELECT 1 FROM user_sessions
            WHERE user_id = ? AND token = ? AND logout_at IS NULL""", (user_id, token))
        return c.fetchone() is not None

    def get_sessions(self, user_id: int, limit: int = 25) -> list:
        c = self.conn.cursor()
        c.execute("""SELECT session_id, login_at, logout_at, actions_count
            FROM user_sessions WHERE user_id = ? ORDER BY login_at DESC LIMIT ?""",
                  (user_id, limit))
        cols = [d[0] for d in c.description]
    ...
    def revoke_session(self, user_id: int, session_id: int):
        c = self.conn.cursor()
        c.execute("""UPDATE user_sessions SET logout_at = CURRENT_TIMESTAMP
            WHERE user_id = ? AND session_id = ? AND logout_at IS NULL""",
                  (user_id, session_id))
        self.conn.commit()
    ...
    def delete_account(self, user_id: int):
        """Right-to-erasure: remove the user and all dependent rows."""
        c = self.conn.cursor()
        c.execute("DELETE FROM fraud_flags WHERE user_id = ?", (user_id,))
        c.execute("DELETE FROM credit_history WHERE user_id = ?", (user_id,))
        c.execute("DELETE FROM loan_applications WHERE user_id = ?", (user_id,))
    ...
    def get_user_data_bundle(self, user_id: int) -> dict:
        """Full personal-data snapshot for GDPR-style export."""
        ...
        txns = [dict(zip(txn_cols, r)) for r in c.fetchall()]
        c.execute("SELECT * FROM savings_goals WHERE user_id = ?", (user_id,))
        g_cols = [d[0] for d in c.description]
        goals = [dict(zip(g_cols, r)) for r in c.fetchall()]
        c.execute("SELECT * FROM loan_applications WHERE user_id = ?", (user_id,))
        l_cols = [d[0] for d in c.description]
        loans = [dict(zip(l_cols, r)) for r in c.fetchall()]
        c.execute("SELECT * FROM feedback WHERE user_id = ?", (user_id,))
        f_cols = [d[0] for d in c.description]
        feedback = [dict(zip(f_cols, r)) for r in c.fetchall()]
        c.execute("SELECT * FROM user_sessions WHERE user_id = ?", (user_id,))
        s_cols = [d[0] for d in c.description]
        sessions = [dict(zip(s_cols, r)) for r in c.fetchall()]
        return {
            "profile": user, "transactions": txns, "savings_goals": goals,
            "loan_applications": loans, "feedback": feedback, "sessions": sessions,
