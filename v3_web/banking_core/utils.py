...
def hash_pin(pin):
    """Salted PBKDF2-SHA256 hash (format: pbkdf2$salt$digest)."""
    salt = secrets.token_hex(16)
    dk = hashlib.pbkdf2_hmac("sha256", str(pin).encode(), salt.encode(), 100_000)
    return f"pbkdf2${salt}${dk.hex()}"


def verify_pin(pin, pin_hash):
    """Verify a PIN against a stored hash. Supports legacy unsalted sha256
    (verified by hash equality) and the new salted pbkdf2 format."""
    if not pin_hash:
        return False
    if pin_hash.startswith("pbkdf2$"):
        try:
            _, salt, expected = pin_hash.split("$")
            dk = hashlib.pbkdf2_hmac("sha256", str(pin).encode(), salt.encode(), 100_000)
            return hmac.compare_digest(dk.hex(), expected)
        except Exception:
            return False
    return hmac.compare_digest(hashlib.sha256(str(pin).encode()).hexdigest(), pin_hash)
